PROGRAM MK_Q2_DFS4
  ...
  INTEGER, PARAMETER ::   &
       ...
       &   jstart = 256    ! RD: ici on corrige tout pour avoir aussi l'antarctique
  ...
  CHARACTER(len=256), PARAMETER :: &
       ...
       &  cf_msk ='/fsnet/data/meom/workdir/dussin/MESHMASKS/lsm_erainterim.nc', &       
       &  cd_str  = '/fsnet/data/meom/workdir/dussin/TMPDIR_TQ2_CORR_ARCTIC/work',  &
       &  cd_dfs4  = '/fsnet/data/meom/workdir/dussin/TMPDIR_TQ2_CORR_ARCTIC/q2_2poles_correct'
  !!
  INTEGER, PARAMETER ::   &
       ni     = 512,    &
       nj     = 256
  ...
  DO jy = jy1, jy2
     ...
     WRITE(cf_in, '(a,"/drowned_",a,"_ERAinterim_y",i4,".nc")') &
...
!              IF ( mask(ji,jj) /= 0 ) THEN
           ...
           DO jj = 213, nj ! RD :antarctic 60 - > 90 S
              !!
              !! Calcule du nouveau q_sat:
              qs_n = q_sat(Xr4tn(ji,jj), Xr4p(ji,jj))*1000.
              !! Calcule du vieux q_sat:
