PROGRAM correc_radlw_ggweights

!------------------------------------------------------
! A program which reads radlw input, applies weights
...
USE NETCDF
   ...
   CHARACTER(LEN=64)                         :: cvar_rad="radlw", cvar_wgt="coefsdlw", cvar_out="radlw"
   ...
   narg= iargc()
   IF ( narg /= 2 ) THEN
      PRINT *,' Usage : correc_radlw_ggweights radlw_file weight_file '
   ...
   CALL check( NF90_INQ_DIMID(ncid_radlw, 'x', xid) )
   CALL check( NF90_INQ_DIMID(ncid_radlw, 'y', yid) )
   CALL check( NF90_INQ_DIMID(ncid_radlw, 'time_counter', tid) )
   ...
   !! Get lon and lat
   CALL check( NF90_OPEN(trim(cfilein), nf90_nowrite, ncid_radlw) )
   CALL check( NF90_INQ_VARID(ncid_radlw, 'nav_lon', lonin_varid) )
   CALL check( NF90_INQ_VARID(ncid_radlw, 'nav_lat', latin_varid) )
   CALL check( NF90_GET_VAR(ncid_radlw, lonin_varid, zlon, start = start, &
              &                 count = count) )
   CALL check( NF90_GET_VAR(ncid_radlw, latin_varid, zlat, start = start, &
              &                 count = count) )
   CALL check( NF90_CLOSE(ncid_radlw) )
