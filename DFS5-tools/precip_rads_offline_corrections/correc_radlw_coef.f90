...
USE NETCDF
   ...
   INTEGER                                   :: ncid_radlw, ncid_wgt, ncid_out  ! netcdf id's
   ...
   CHARACTER(LEN=256), PARAMETER             :: cfileout="radlw_corrected.nc"
   CHARACTER(LEN=64)                         :: cvar_rad="radlw", cvar_wgt="radlw", cvar_out="radlw"
   ...
   narg= iargc()
   IF ( narg /= 2 ) THEN
      PRINT *,' Usage : correc_radlw_coef radlw_file coef_file '
   ...
   CALL check( NF90_OPEN(trim(cfilein), nf90_nowrite, ncid_radlw) )

   CALL check( NF90_INQ_DIMID(ncid_radlw, 'lon0', xid) )
   CALL check( NF90_INQ_DIMID(ncid_radlw, 'lat0', yid) )
   CALL check( NF90_INQ_DIMID(ncid_radlw, 'time', tid) )

   CALL check( NF90_INQUIRE_DIMENSION(ncid_radlw, xid,len=npx) )
   CALL check( NF90_INQUIRE_DIMENSION(ncid_radlw, yid,len=npy) )
   CALL check( NF90_INQUIRE_DIMENSION(ncid_radlw, tid,len=npt) )

   PRINT *, '-------------------------------------------------------------'
   PRINT *, 'in radlw file : npx = ', npx
   PRINT *, 'in radlw file : npy = ', npy
   PRINT *, 'in radlw file : npt = ', npt

   CALL check( NF90_CLOSE(ncid_radlw) )
   ...
   !! Get lon and lat
   CALL check( NF90_OPEN(trim(cfilein), nf90_nowrite, ncid_radlw) )
   CALL check( NF90_INQ_VARID(ncid_radlw, 'lon0', lonin_varid) )
   CALL check( NF90_INQ_VARID(ncid_radlw, 'lat0', latin_varid) )
   CALL check( NF90_GET_VAR(ncid_radlw, lonin_varid, zlon0 ) )
   CALL check( NF90_GET_VAR(ncid_radlw, latin_varid, zlat0 ) )
   CALL check( NF90_CLOSE(ncid_radlw) )
   ...
   CALL check( nf90_put_att(ncid_out, radout_varid, "long_name", "Longwave radiation") )
   ...
   ! radlw
   CALL check( NF90_OPEN(trim(cfilein), nf90_nowrite, ncid_radlw) )
   CALL check( NF90_INQ_VARID(ncid_radlw, trim(cvar_rad), radin_varid) )
   ! weight
   CALL check( NF90_OPEN(trim(cwgtin), nf90_nowrite, ncid_wgt) )
   ...
   DO irec=1,npt
      ...
      start(3) = irec
      CALL check( NF90_GET_VAR(ncid_radlw, radin_varid, zradin, start = start, &
                  &                 count = count) )
